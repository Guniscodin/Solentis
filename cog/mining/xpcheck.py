    ...
    async def example(self, interaction: discord.Interaction):
        ...
        try:
            if os.path.exists(f"{interaction.user.id}.json"):
                with open(f"{interaction.user.id}.json", "r") as f:
                    data = json.load(f)
                    main_embed = discord.Embed(
                        title="Current Xp check...",
                        description=f"You need **{data['xp']}/{data['xp_required']}** for the next lvl",
                        color=discord.Color.dark_blue()
                    )
                    main_embed.set_footer(
                        text="Keep mining by typing /mine",
            ...
            logging.error(f"Xp_check error: {e}")

async def setup(bot: commands.Bot):
    await bot.add_cog(xpcheck(bot))
